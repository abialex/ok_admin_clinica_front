...
class AsistenteApi implements BaseApi {
  ...
  Future<Either<String, List<AsistentesDto>>> getAsistentes() async {
    ...
    return ApiUtils.reponseHandler(response, (data) => (data as List).map((e) => AsistentesDto.fromJson(e)).toList());
  }

  Future<Either<String, AsistenteDto>> getAsistentessById(int idasistente) async {
    final response = await _dio.get<ApiModel>(
      "${url}asistente/$idasistente/",
    );
    return ApiUtils.reponseHandler(response, (data) => AsistenteDto.fromJson(data));
  }

  Future<Either<String, AsistenteCredentialsDto>> createAsistente(AsistenteCreateModel doctorCreateModel) async {
    final response = await _dio.post<ApiModel>(
      "${url}asistente/",
      data: doctorCreateModel.toJson(),
    );
    return ApiUtils.reponseHandler(response, (data) => AsistenteCredentialsDto.fromJson(data));
  }

  Future<Either<String, int>> updateAsistente(AsistenteUpdateModel doctorUpdateModel) async {
